...
# See HaplotypePhasing.sh script
...
do
	plink --chr-set 28 no-xy no-mt --recode12 \
	--vcf beagle/best66.chr$i.beagle.phase.vcf \
	--out hap/best66.chr$i.beagle.phase
done

for i in $(seq 1 1 15; seq 17 1 28)
do
	perl /scratch/snyder/m/mathur20/MQU/ch3/softwares/fs_4.1.1/plink2chromopainter.pl \
	-p=hap/best66.chr$i.beagle.phase.ped \
	-m=hap/best66.chr$i.beagle.phase.map \
	-o=fineS/best66.chr$i.beagle.phase
done
...
do
	...
	-D cdf -U c \
	fineS/best66.chr$i.beagle.phase recMap/chicken.chr$i.recMap.txt recMap/chicken.chr$i.recombfile
done
...
do
	...
	best66.chr$i.cp \
	-n -hpc 1 \
	-phasefiles /scratch/snyder/m/mathur20/MQU/ch3/structure/fineS/run2/input/fineS/best66.chr$i.beagle.phase \
...
for i in $(seq 1 1 15; seq 17 1 28)
do
	cd /scratch/snyder/m/mathur20/MQU/ch3/structure/fine_structure/final/painter/best66.auto.long.chr$i/commandfiles/
...
do
	count=1
	cat best66.noSing.chr$i/commandfiles/commandfile1a.txt | while read -r line
	do
		echo "#!/bin/sh -l
#SBATCH -A fnrfish
#SBATCH -N 1
...
for i in $(seq 1 1 15; seq 17 1 28)
do
	/scratch/snyder/m/mathur20/MQU/ch3/softwares/fs_4.1.1/fs best66.chr$i.cp -go
done
...
do
	count=1
	cat best66.chr$i/commandfiles/commandfile2.txt | while read -r line
	do
		echo "#!/bin/sh -l
#SBATCH -A fnrfish
#SBATCH -N 1
...
for i in $(seq 1 1 15; seq 17 1 28)
do
	line=$(head -n 1 best66.chr$i/commandfiles/commandfile3.txt) 
...
do
	count=1
	cat best66.chr$i/commandfiles/commandfile4.txt | while read -r line
	do
		echo "#!/bin/sh -l
#SBATCH -A fnrfish
#SBATCH -N 1
